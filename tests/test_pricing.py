...
def test_deepseek_tier_prices_are_unchanged():
    table = PriceTable()
    assert table.cost("deepseek-v4-flash", TokenUsage(input_tokens=1_000_000)) == 0.140
    assert table.cost("deepseek-v4-flash", TokenUsage(output_tokens=1_000_000)) == 0.280
    assert table.cost("deepseek-v4-pro", TokenUsage(input_tokens=1_000_000)) == 0.435
    assert table.cost("deepseek-v4-pro", TokenUsage(output_tokens=1_000_000)) == 0.870
    assert (
        table.cost(
            "deepseek-v4-flash",
...
def test_monthly_role_route_scenario():
    """Reproduce the role-aware $72.20 monthly API scenario from the README."""
    table = PriceTable()
    ...
    assert round(text + normal + conflict + macro, 2) == 72.20
